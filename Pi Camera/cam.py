...
def takeVideo():
	...
	from signal import pause
	import signal
	global saveIdx, storeMode, busy, screenMode
	while True:
		filename = pathData[storeMode] + '/MOV_' + '%04d' % saveIdx + '.h264'
		if not os.path.isfile(filename):break
		saveIdx += 1
		if saveIdx > 9999: saveIdx = 0
	stoprec = Button(27)
...
def buttonpress():
	global saveIdx, screenMode, loadIdx, scaled, screenModePrior
	GPIO.setmode(GPIO.BCM)
	GPIO.setup(17, GPIO.IN, pull_up_down=GPIO.PUD_UP)
	GPIO.setmode(GPIO.BCM)
	GPIO.setup(22, GPIO.IN, pull_up_down=GPIO.PUD_UP)
	while True:
		shutter_state = GPIO.input(17)
		vid_state = GPIO.input(22)
		if shutter_state == False:
			takePicture()
		elif vid_state == False:
			takeVideo()
		else:
			break
	GPIO.cleanup()
